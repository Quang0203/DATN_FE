...
export class ProfileComponent implements OnInit {
  ...
  private loadProfile() {
    this.profileService.getProfile().subscribe({
      next: res => {
        ...
        // Hiển thị ảnh bằng lái nếu có
        this.drivingLicensePreview = this.profileData.drivinglicense || null;
  ...
  handleSubmitProfile() {
    ...
    if (this.profileData.dateofbirth) {
      const dob = new Date(this.profileData.dateofbirth);
      ...
      if (age < 18) {
        alert('Bạn phải đủ 18 tuổi trở lên để cập nhật hồ sơ.');
        return;
      }
    }
  ...
  async onFileSelected(event: any) {
    ...
    if (file) {
      ...
      formData.append('upload_preset', 'datnupload');
      formData.append('folder', 'DrivingLicense');
      try {
        const resp = await fetch('https://api.cloudinary.com/v1_1/quangkedo/image/upload', {
          method: 'POST',
          body: formData
        });
        const body = await resp.json();
        this.profileData.drivinglicense = body.secure_url;
        this.drivingLicensePreview = body.secure_url;
      } catch (err) {
        alert('Có lỗi xảy ra khi upload ảnh bằng lái.');
      } finally {
